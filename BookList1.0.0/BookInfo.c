...
int GetStrTo(char* szDestination,
	...
	for (int i = 0; i < nSizeOfDestination+1; ++i) {
...
int ReadBookInfoFromFile(TbookInfo* info, FILE* pfInput) {
	...
	if (!(nIndexOfBuffer = GetStrTo(info->szAuthor, SIZE_NAMES, '|', IsAuthorChar, szBuffer))) return -1;
	//перевірка формату імені, видалення лишніх пробілів, якщо формат не правильний, якщо щось пішло не так ...
	if (!FormatAuthor(info->szAuthor)) return -1;
	...
	if (!(nIndexLocalBuffer = GetStrTo(info->szName, SIZE_NAMES, '|', IsNameBookChar, szBuffer + nIndexOfBuffer))) return -1;
	...
	char szTmpForDigits[MAX_LENGHT_OF_NUMBER+4] = { 0 };
	...
	info->nYear = atoi(szTmpForDigits);
	...
	nIndexOfBuffer += nIndexLocalBuffer + 1;
	info->nPages = atoi(szTmpForDigits);
	if (info->nPages <= 0) return -1;

	if (!(nIndexLocalBuffer = GetStrTo(
		szTmpForDigits,
		MAX_LENGHT_OF_NUMBER +3,
		'\n',
		IsFloatDiginChar,
		szBuffer + nIndexOfBuffer))) return -1;
	...
	info->dPrice = atof(szTmpForDigits);
	return 1;
}
//--------------------------------------------------------------------------------
...
{
	if (strcmp(info1.szAuthor, info2.szAuthor)
		|| strcmp(info1.szName, info2.szName)
		|| info1.nYear != info2.nYear
		|| info1.nPages != info2.nPages
		|| info1.dPrice != info2.dPrice) return 0;
	return 1;
}
...
void PrintBook(TbookInfo info) {
	printf("|%-*s|", SIZE_OF_COL_WITH_NAMES, info.szAuthor);
	printf("%-*s|", SIZE_OF_COL_WITH_NAMES, info.szName);
	printf("%*i|", SIZE_OF_COL_WITH_NUMBERS, info.nYear);
	printf("%*i|", SIZE_OF_COL_WITH_NUMBERS, info.nPages);
	printf("%*.2lf|\n", SIZE_OF_COL_WITH_NUMBERS, info.dPrice);
}
//--------------------------------------------------------------------------------
